...
export class VerComponent implements OnInit{
  ...
  constructor(private route:ActivatedRoute,private proveedorServicio:ProveedorService,private itemServicio:ItemService,private toastr: ToastrService,private dialog:MatDialog,private dialogo:MatDialog){
  ...
  agregar() {
    let item: Item={id:0,  proveedor_id:0,producto_id:0,nuevo:""};
    const dialogo1 = this.dialog.open(FormularioItemComponent, {data:{item:item,texto:"Nuevo Item"}});
  ...
  actualizar(item:Item):void {
    let prov:Item
    const dialogo1 = this.dialog.open(FormularioItemComponent, {data:{item:item,texto:"Editar Item"}});
    dialogo1.afterClosed().subscribe(art => {
      if (art != undefined){
        prov={
          id:item.id,
